...
class dbcon{
		...
		public $veritabani = "akdrentacar_db"; //veritabanı adı
		public $host="localhost";
		public $hesap="akdrentacar_db"; //kullanıcı adı
		public $sifre="veriTabanYgt621.akdrentacar_db"; //sifre
		public $baglan;
		...
		function getByIds($table, $ids = [])
		{
			$placeholders = str_repeat('?,', count($ids) - 1) . '?';
			$yenisorgu = "SELECT * FROM $table WHERE id IN ($placeholders)";
			$stuncek = $this->baglan->prepare($yenisorgu);
			$stuncek->execute($ids);
			return $stuncek->fetchAll(PDO::FETCH_OBJ);
		}

		function getById($table, $id)
		{
						$dondur =NULL;
						$yenisorgu = "SELECT * FROM $table WHERE id=?";
						$stuncek = $this->baglan->prepare($yenisorgu);
						$stuncek->execute(array($id));
						return $stuncek->fetch(PDO::FETCH_OBJ);
		}
